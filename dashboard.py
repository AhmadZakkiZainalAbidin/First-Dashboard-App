...
with st.expander("See explanation"):
    st.write(
        """This dashboard contains analysis based on data sets from 2016 to 2018. Contains product categories along with information on orders and income earned. The product information presented is based on review scores and average selling price information. And finally, customer analysis which contains tables and customer categories based on RFM analysis which has been normalized based on the RFM equation in link bellow

        https://www.geeksforgeeks.org/rfm-analysis-analysis-using-python/
        """
    )
    st.write( )
    st.write("It should be noted that the customer category subsection is not an all time explanation so it is the result of the entire data (not sidebar time input). Meanwhile, other sub categories will change over time as the sidebar input.")
    st.write()
    st.write("""If you curios about my dataset that I using and streamlite for the dashboar you can check in link bellow
             
             https://github.com/AhmadZakkiZainalAbidin/ProjectAdataAnalyst.git
             """)
...
with st.container() :
    ...
    st.markdown(f"<h7 style='text-align: center; color: black;'>Lastest By Recency </h7>", unsafe_allow_html=True)
    st.table(data=customer_recency.sort_values(by='recency', ascending=True).head().reset_index())

    st.markdown(f"<h7 style='text-align: center; color: black;'>Top Customer By Frequency </h7>", unsafe_allow_html=True)
    st.table(data=customer_frequency.sort_values(by='frequency', ascending=False).head().reset_index())

    st.markdown(f"<h7 style='text-align: center; color: black;'>Top Customer By Monetary </h7>", unsafe_allow_html=True)
    st.table(data=customer_monetary.sort_values(by='monetary', ascending=False).head().reset_index())
with st.container() :
